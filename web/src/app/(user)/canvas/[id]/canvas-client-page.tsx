...
function agentLockedTargetIds(action: CanvasAgentAction, nodes: CanvasNodeData[], connections: CanvasConnection[]) {
    const stringId = (key: string) => (typeof action.arguments[key] === "string" ? (action.arguments[key] as string) : "");
    const stringIds = (key: string) => (Array.isArray(action.arguments[key]) ? (action.arguments[key] as unknown[]).filter((value): value is string => typeof value === "string") : []);
    const targets = new Set<string>();
    if (action.name === "update_text_node" || action.name === "update_node" || action.name === "delete_node") {
        const nodeId = stringId("nodeId");
        if (nodeId) targets.add(nodeId);
        if (action.name === "delete_node") {
            const target = nodes.find((node) => node.id === nodeId);
            target?.metadata?.batchChildIds?.forEach((id) => targets.add(id));
            if (target?.type === CanvasNodeType.Group) nodes.filter((node) => node.metadata?.groupId === nodeId).forEach((node) => targets.add(node.id));
        }
    }
    if (action.name === "create_group") stringIds("nodeIds").forEach((id) => targets.add(id));
    if (action.name === "create_connection") {
        targets.add(stringId("fromNodeId"));
        targets.add(stringId("toNodeId"));
    }
    if (action.name === "delete_connection") {
        const connection = connections.find((item) => item.id === stringId("connectionId"));
        if (connection) {
    ...
    if (action.name === "arrange_nodes") {
        const requested = stringIds("nodeIds");
        const arranged = requested.length ? nodes.filter((node) => requested.includes(node.id)) : nodes.filter((node) => !node.metadata?.groupId && !node.metadata?.batchRootId);
        arranged.forEach((node) => {
            targets.add(node.id);
            if (node.type === CanvasNodeType.Group) nodes.filter((child) => child.metadata?.groupId === node.id).forEach((child) => targets.add(child.id));
        });
    }
    return Array.from(targets).filter((id) => nodes.find((node) => node.id === id)?.collaboration?.locked);
}
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const collaborationRef = useRef<CanvasCollaborationState>(canvasCollaborationAdapter.createState());
    ...
    const [collaboration, setCollaboration] = useState<CanvasCollaborationState>(() => collaborationRef.current);
    ...
    const hasRecentAgentNodes = nodes.some((node) => {
        const changedAt = Date.parse(node.collaboration?.lastAgentChangedAt || "");
        return node.collaboration?.lastEditedBy === "agent" && Number.isFinite(changedAt) && canvasNow - changedAt < 9_000;
    });
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            ...
            const restoredCollaboration = canvasCollaborationAdapter.normalize(project.collaboration);
            setNodes(restoredNodes);
            setConnections(project.connections);
            collaborationRef.current = restoredCollaboration;
            activeAgentBatchRef.current = null;
            setCollaboration(restoredCollaboration);
    ...
    useEffect(() => {
        ...
        updateProject(projectId, { nodes, connections, chatSessions, activeChatId, agentConfig, backgroundMode, showImageInfo, collaboration });
    }, [activeChatId, agentConfig, backgroundMode, chatSessions, collaboration, connections, nodes, projectId, projectLoaded, showImageInfo, updateProject]);
    ...
    const commitCollaborationState = useCallback((next: CanvasCollaborationState) => {
        collaborationRef.current = next;
        setCollaboration(next);
    }, []);

    const commitHumanNodeMutation = useCallback(
        (nodeIds: Iterable<string>, updater: (current: CanvasNodeData[]) => CanvasNodeData[]) => {
            const changedIds = Array.from(new Set(nodeIds));
            const nextNodes = updater(nodesRef.current);
            const marked = canvasCollaborationAdapter.markHumanNodes(collaborationRef.current, nextNodes, changedIds);
            nodesRef.current = marked.nodes;
            setNodes(marked.nodes);
            commitCollaborationState(marked.state);
        },
        [commitCollaborationState],
    );

    const markHumanNodeCommit = useCallback(
        (nodeIds: Iterable<string>) => {
            const marked = canvasCollaborationAdapter.markHumanNodes(collaborationRef.current, nodesRef.current, nodeIds);
            nodesRef.current = marked.nodes;
            setNodes(marked.nodes);
            commitCollaborationState(marked.state);
        },
    ...
    const deleteNodes = useCallback(
        (ids: Set<string>, options?: { actor?: "human" | "agent" }) => {
            if (!ids.size) return;
            ...
            const marked = options?.actor === "agent" ? { nodes: nextNodes, state: collaborationRef.current } : canvasCollaborationAdapter.markHumanNodes(collaborationRef.current, nextNodes, allIds);
            nodesRef.current = marked.nodes;
            connectionsRef.current = nextConnections;
            selectedNodeIdsRef.current = new Set();
            setNodes(marked.nodes);
            setConnections(nextConnections);
            if (options?.actor !== "agent") commitCollaborationState(marked.state);
        ...
        [chatSessions, cleanupCanvasFiles, commitCollaborationState, deleteCanvasTaskRecords, projectId],
    );

    const deleteConnection = useCallback(
        (connectionId: string, options?: { actor?: "human" | "agent" }) => {
            ...
            connectionsRef.current = nextConnections;
            setConnections(nextConnections);
            if (removed && options?.actor !== "agent") markHumanNodeCommit([removed.fromNodeId, removed.toNodeId]);
    ...
    const clearCanvas = useCallback(() => {
        const removedNodeIds = nodesRef.current.map((node) => node.id);
        deleteCanvasTaskRecords();
        nodesRef.current = [];
        connectionsRef.current = [];
        setNodes([]);
        setConnections([]);
        const marked = canvasCollaborationAdapter.markHumanNodes(collaborationRef.current, [], removedNodeIds);
        commitCollaborationState(marked.state);
        setInfoNodeId(null);
        setCropNodeId(null);
    ...
    }, [cleanupCanvasFiles, commitCollaborationState, deleteCanvasTaskRecords, deselectCanvas, projectId]);
    ...
    const createGroupFromSelection = useCallback(
        (nodeIds?: string[], options?: { actor?: "human" | "agent" }) => {
            ...
            const marked = options?.actor === "agent" ? { nodes: nextNodes, state: collaborationRef.current } : canvasCollaborationAdapter.markHumanNodes(collaborationRef.current, nextNodes, [...selectedIds, group.id]);
            nodesRef.current = marked.nodes;
    ...
    const finishNodeDrag = useCallback(
        (clientX?: number, clientY?: number) => {
            ...
            if (dragRef.current.hasMoved && clientX != null && clientY != null) {
                ...
                commitHumanNodeMutation(movedIds, (prev) => {
    ...
    const appendImportedImageNode = useCallback(
        (image: UploadedImage, title: string, position: Position, type: CanvasNodeType.Image | CanvasNodeType.Panorama) => {
            ...
            commitHumanNodeMutation([id], (prev) => [...prev, newNode]);
            setSelectedNodeIds(new Set([id]));
            setSelectedConnectionId(null);
            setDialogNodeId(id);
        },
                ...
                prev.map((node) =>
                    node.id === nodeId
                        ? {
                              ...
                              collaboration: {
                                  ...node.collaboration,
                                  revision: node.collaboration?.revision || collaborationRef.current.revision,
                                  lastEditedBy: "human",
    ...
    const uploadNodeMediaToCloud = useCallback(
        async (node: CanvasNodeData) => {
            ...
            try {
                ...
                commitHumanNodeMutation([node.id], (nodes) =>
                    nodes.map((item) =>
                        item.id === node.id
                            ? {
                                  ...item,
    ...
    const cropImageNode = useCallback(
        async (node: CanvasNodeData, crop: CanvasImageCropRect) => {
            ...
            commitHumanNodeMutation([node.id, childId], (prev) => [...prev, child]);
    ...
    const handleImageInputChange = useCallback(
            ...
            if (target?.nodeId) {
                ...
                try {
                    if (isAudioFile(file)) {
                        const audio = await uploadMediaFile(file, "audio");
                        const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
                        commitHumanNodeMutation([target.nodeId], (prev) =>
                            prev.map((node) =>
        ...
        [commitHumanNodeMutation, createAudioFileNode, createImageFileNode, createVideoFileNode, screenToCanvas, size.height, size.width],
    );
    ...
    const startCanvasAgentBatch = useCallback(
        ({ batchId, summary }: { batchId: string; summary: string }) => {
            ...
            commitCollaborationState(started.state);
        },
        [commitCollaborationState],
    );
    ...
    const updateCanvasAgentProgress = useCallback(
        ({ batchId, message: progressMessage }: { batchId: string; message: string }) => {
            ...
            commitCollaborationState(canvasCollaborationAdapter.updateStatus(collaborationRef.current, progressMessage));
        },
        [commitCollaborationState],
    );
    ...
    const noteCanvasAgentActionResult = useCallback(({ batchId, result }: { batchId: string; action: CanvasAgentAction; result: CanvasAgentToolResult }) => {
        ...
        if (!runtime || runtime.id !== batchId || result.ok) return;
        const error = typeof result.message === "string" ? result.message : "Agent 操作失败";
        if (!runtime.errors.includes(error)) runtime.errors.push(error);
    ...
    const finishCanvasAgentBatch = useCallback(
        ({ batchId, error }: { batchId: string; error?: string }) => {
            ...
            if (collaborationRef.current.status.state === "conflict" && collaborationRef.current.status.batchId === batchId) {
                runtime.hadConflict = true;
                const conflictMessage = collaborationRef.current.status.message;
                if (!runtime.errors.includes(conflictMessage)) runtime.errors.push(conflictMessage);
            }
            const finished = canvasCollaborationAdapter.finishBatch({
                state: collaborationRef.current,
                runtime,
                nodes: nodesRef.current,
                connections: connectionsRef.current,
            ...
            activeAgentBatchRef.current = null;
            commitCollaborationState(finished.state);
        },
        [commitCollaborationState],
    );

    const undoLatestCanvasAgentBatch = useCallback(() => {
        const result = canvasCollaborationAdapter.undoLatest(collaborationRef.current, nodesRef.current, connectionsRef.current);
        if (!result.ok) {
        ...
        commitCollaborationState(result.state);
        message.success("已撤销最近一个可逆 Agent 批次");
    }, [commitCollaborationState, message]);
    ...
    const getCanvasAgentContext = useCallback(
        (agentState: CanvasAgentState) =>
            buildCanvasAgentContext({
                ...
                agentState,
                collaborationRevision: collaborationRef.current.revision,
            }),
    ...
    const executeCanvasAgentAction = useCallback(
        async (action: CanvasAgentAction, messageReferenceNodeIds: string[]): Promise<CanvasAgentToolResult> => {
            ...
            const isAgentWriteAction = CANVAS_AGENT_WRITE_ACTIONS.has(action.name);
            const batchRuntime = activeAgentBatchRef.current;
            const beforeNodes = nodesRef.current;
            const beforeConnections = connectionsRef.current;
            if (isAgentWriteAction) {
                if (!batchRuntime) return { ok: false, code: "agent_batch_missing", message: "Agent 变更批次尚未开始，未修改画布" };
                const guard = canvasCollaborationAdapter.guardMutation(collaborationRef.current, batchRuntime, agentLockedTargetIds(action, beforeNodes, beforeConnections));
                if (!guard.ok) {
                    const recorded = canvasCollaborationAdapter.recordAction({
                        ...
                        afterConnections: beforeConnections,
                        result: guard,
                    });
                    commitCollaborationState(recorded.state);
                    return guard;
                }
            }
            ...
            const actionResult = await (async (): Promise<CanvasAgentToolResult> => {
                try {
                    ...
                    if (action.name === "get_canvas_summary") {
                        return {
                            ok: true,
                            project: { id: projectId, title: currentProject?.title || "未命名画布", revision: collaborationRef.current.revision },
                    ...
                    if (action.name === "delete_node") {
                        ...
                        deleteNodes(new Set([nodeId]), { actor: "agent" });
                    ...
                    if (action.name === "delete_connection") {
                        ...
                        deleteConnection(connectionId, { actor: "agent" });
                    ...
                    if (action.name === "create_group") {
                        ...
                        if (missingNodeId) return missingNodeResult(missingNodeId);
                        const groupId = createGroupFromSelection(nodeIds, { actor: "agent" });
                    ...
                    if (action.name === "generate_image" || action.name === "edit_image" || action.name === "generate_video" || action.name === "generate_audio") {
                        ...
                        const layoutSourceNodes =
                            mode === "image"
                                ? []
                                : mode === "video"
                                  ? nodesRef.current.filter((node) => !node.metadata?.groupId && (node.type === CanvasNodeType.Text || isCanvasImageNodeType(node.type) || node.type === CanvasNodeType.Group))
                                  : sourceNodes;
            ...
            if (isAgentWriteAction && batchRuntime) {
                const interleavedHumanChange = collaborationRef.current.revision !== batchRuntime.expectedRevision;
                const recorded = canvasCollaborationAdapter.recordAction({
                    state: collaborationRef.current,
                    runtime: batchRuntime,
                    actionName: action.name,
                    beforeNodes,
                    beforeConnections,
                    afterNodes: nodesRef.current,
                    afterConnections: connectionsRef.current,
                    result: actionResult,
                });
                activeAgentBatchRef.current = recorded.runtime;
                if (recorded.nodes !== nodesRef.current) {
                    nodesRef.current = recorded.nodes;
                    setNodes(recorded.nodes);
                }
                commitCollaborationState(recorded.state);
                if (interleavedHumanChange && actionResult.ok) {
                    return {
                        ok: false,
                        code: "revision_conflict",
                        message: recorded.state.status.message,
                        appliedNodeIds: Array.from(recorded.runtime.affectedNodeIds),
                    };
                }
            }
            return actionResult;
        },
        [agentEffectiveConfig, commitCollaborationState, createGroupFromSelection, currentProject?.title, deleteConnection, deleteNodes, getCanvasCenter, handleGenerateNode, isAiConfigReady, projectId, renameProject, updateProject],
    );
    ...
    const runLocalCollaborationDemo = useCallback(async () => {
        ...
        try {
            const action: CanvasAgentAction = {
                ...
                arguments: {
                    title: "Agent 协作演示",
                    content: "这是本地 mock 创建的可编辑节点。你可以锁定、拖拽或修改文字，再检查 revision 冲突与批次撤销。",
                    sourceNodeIds: [],
                },
            };
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            now={
                                ...
                                node.collaboration?.lastAgentChangedAt
                                    ? canvasNow
                                    : undefined
                            }
...
function canvasAgentNodeSummary(node: CanvasNodeData) {
    ...
    return {
        ...
        lockedByHuman: Boolean(node.collaboration?.locked),
        revision: node.collaboration?.revision || 0,
        lastEditedBy: node.collaboration?.lastEditedBy,
    };
}
