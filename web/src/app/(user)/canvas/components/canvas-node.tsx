...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const isLocked = Boolean(data.collaboration?.locked);
    const lastAgentChangedAt = Date.parse(data.collaboration?.lastAgentChangedAt || "");
    const wasRecentlyChangedByAgent = Boolean(data.collaboration?.lastEditedBy === "agent" && now && Number.isFinite(lastAgentChangedAt) && now - lastAgentChangedAt < 9_000);
