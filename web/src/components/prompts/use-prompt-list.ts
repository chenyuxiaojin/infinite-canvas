...
export function usePromptList({ keyword, tags, category, enabled = true, favorites = false }: { keyword: string; tags: string[]; category: string; enabled?: boolean; favorites?: boolean }) {
    const query = useInfiniteQuery({
        queryKey: ["prompts", keyword, tags, category, favorites],
        queryFn: ({ pageParam, signal }) => fetchPrompts({ keyword, tag: tags, category, page: pageParam, pageSize: PROMPT_PAGE_SIZE, favorites, signal }),
        initialPageParam: 1,
